...
\begin{itemize}
	...
	\item Grading details
	      \begin{itemize}
		      \item Exams - 40, Final - 20, Textbook Homework - 15, Online Homework - 15, Quizzes - 10
		      \item Textbook Homework
          \item Online Homework
		      \item Quizzes
	      \end{itemize}
	\item Importance of reading and learning on your own
  \item The AI talk: \emph{Mastery} takes work!
	...
	\item Show how to access Canvas
	      \begin{itemize}
		      ...
		      \item Homeworks and quizzes
	      \end{itemize}
\end{itemize}

% \chapter*{Textbook Errata}
% All errata I had found have been corrected in the 11th ed.
...
\begin{itemize}
	\item Changes in state functions can be found without dependence on the path from initial to final states
...
\begin{itemize}
    ...
    $S_m(T_2) = S_m(T_1) + \displaystyle\int_{T_1}^{T_2}\dfrac{C_{P,m}(T)}{T}\mathrm{d}T + \displaystyle\sum\limits_{transitions}\dfrac{\Delta_{trs}H}{T_{trs}}$
  \item We can also use this equation for entropies of change (i.e. for a reaction entropy)

    $\Delta S(T_2) = \Delta S(T_1) + \displaystyle\int_{T_1}^{T_2}\dfrac{\Delta C_{P}(T)}{T}\mathrm{d}T \approx \Delta S(T_1) + \Delta C_{P}\ln\dfrac{T_2}{T_1}$ 

    (assuming no phase changes and negligible changes to $C_P$ over the temperature range)
  \item If we can use the entropy at absolute $0$ as $T_1$, then we can get to an absolute entropy at any temperature
...
\begin{itemize}
  \item \emph{EVERYTHING} is entropy (crazy string board meme)
  \item The temperature dependence of $G$ is ultimately due to entropy ($\Delta H$ only matters because of its effect on $S_{surr}$)
  \item Remember the Spheres Demo (system evolves to macrostates with most numerous microstates)
  \item Schr\"odinger's essay ``What is Life?'' and low entropy of food
  \item Getting a bit existential (just for a moment) -- Life, death and the struggle against entropy
\end{itemize}
...
\begin{itemize}
  ...
  \item Since phase changes along the boundary are reversible, we can replace $\Delta_{trs}S=\dfrac{\Delta_{trs}H}{T}$, giving $\dfrac{\mathrm{d}p}{\mathrm{d}T}= \dfrac{\Delta_{trs}H}{T\Delta_{trs}V_m}$, which rearranges to (after integrating): $\Delta p = \dfrac{\Delta H}{\Delta V}\ln\dfrac{T_2}{T_1}$
...
\begin{itemize}
	...
	\item A third type, $\lambda$ transitions, have $C_p$ approach $\infty$ at the transition, but does so via asymptotes rather than discontinuously (Figure 4A.12)
...
\begin{itemize}
	...
	\item Adding $20~ml$ ($0.343 mol$) of ethanol to $20~ml$ ($1.11 mol$) water will give a final volume substantially lower than $40~ml$
	\item The total change in volume is: ($-1.7 ml$)
\end{itemize}
...
\begin{itemize}
  ...
  \item Now, this equation can be differentiated to give: $\mathrm{d}G = \mu_A\mathrm{d}n_A + n_A\mathrm{d}\mu_A + \mu_B\mathrm{d}n_B + n_B\mathrm{d}\mu_B+\ldots$
  ...
  \item For a 2-component system it can be expressed as: $\mathrm{d}\mu_B = -\dfrac{n_A}{n_B}\mathrm{d}\mu_A$
\end{itemize}
